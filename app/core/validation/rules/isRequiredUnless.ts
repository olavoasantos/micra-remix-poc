...
export interface IsRequiredUnlessOptions {
  message?: string;
  field: string;
  value?: any;
}

export function isRequiredUnless<Type>(
  options: IsRequiredUnlessOptions,
  rule: TypeGuard<Type>,
) {
  const guard = typeGuard<Type | undefined>(
    ...
    function requiredIfGuard(context) {
      ...
      const message =
        options.message ||
        `The ${key} field is required when ${options.field} is not ${
          options.value ?? 'set'
        }.`;
      ...
      if (
        ...
        (!('value' in options) && !valueFromPath)
      ) {
